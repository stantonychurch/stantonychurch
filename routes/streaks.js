...
router.get('/:memberId', authenticateToken, async (req, res) => {
    ...
    if (!streak) {
        (await db.query('INSERT OR IGNORE INTO member_streaks (member_id) VALUES (?)', [req.params.memberId]))[0];
...
router.post('/update', authenticateToken, async (req, res) => {
    try {
        ...
        (await db.query('INSERT OR IGNORE INTO member_streaks (member_id) VALUES (?)', [member_id]))[0];
